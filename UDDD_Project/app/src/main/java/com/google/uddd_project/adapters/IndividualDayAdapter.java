...
public class IndividualDayAdapter extends Adapter<IndividualDayAdapter.ViewHolder> {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public FAImageView FAImg;
        public RelativeLayout rlt;

        public ViewHolder(View view) {
            super(view);
            this.tvName = (TextView) view.findViewById(R.id.exerciseName);
            this.tvNum = (TextView) view.findViewById(R.id.rotation);
            this.FAImg = (FAImageView) view.findViewById(R.id.animation);
            this.rlt = (RelativeLayout) view.findViewById(R.id.cardViewInRecycler);
        }
    }
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, int i) {
        ...
        if (i < this.workoutDataList.size()) {
            viewHolder.rlt.setVisibility(View.VISIBLE);
            viewHolder.FAImg.setInterval(1000);
            viewHolder.FAImg.setLoop(true);
            viewHolder.FAImg.reset();
            for (int addImageFrame : ((WorkoutData) this.workoutDataList.get(i)).getImageIdList()) {
                viewHolder.FAImg.addImageFrame(addImageFrame);
            }
            viewHolder.FAImg.startAnimation();
            viewHolder.tvName.setText(((WorkoutData) this.workoutDataList.get(i)).getExcName().replace("_", " ").toUpperCase());
        ...
        viewHolder.rlt.setVisibility(View.GONE);
    }
